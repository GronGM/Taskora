...
export default function ThemeToggle({ className = '' }) {
    ...
    return (
        <label className={`inline-flex min-w-0 items-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-300 ${className}`}>
            ...
            <select
                ...
                aria-label={`Тема оформления. Сейчас: ${options.find((option) => option.value === preference)?.label ?? 'Как в системе'}, отображается ${resolvedTheme === 'dark' ? 'темная' : 'светлая'} тема`}
